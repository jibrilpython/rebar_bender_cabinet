...
class _AddScreenState extends ConsumerState<AddScreen> {
  ...
  void dispose() {
    _pageCtrl.dispose();
    for (final c in [_catIdCtrl, _toolNameCtrl, _mfrCtrl, _countryCtrl,
        _eraCtrl, _modelCtrl, _serialCtrl, _rebarCapCtrl, _bendAngleCtrl,
        _materialCtrl, _weightCtrl, _provenanceCtrl, _notesCtrl, _tagsCtrl]) {
      c.dispose();
    }
    super.dispose();
  }
      ...
      child: Row(children: [
        GestureDetector(
          ...
          child: Container(
            ...
            child: Icon(Icons.arrow_back_rounded, color: kSecondaryText, size: 18.r),
          ),
        ),
        SizedBox(width: 16.w),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(widget.isEdit ? 'EDIT TOOL' : 'NEW TOOL', style: GoogleFonts.barlow(
            fontSize: 10.sp, fontWeight: FontWeight.w700, color: kAccent, letterSpacing: 2.5,
          )),
          Text(labels[_currentPage], style: GoogleFonts.barlow(
            fontSize: 18.sp, fontWeight: FontWeight.w800, color: kPrimaryText,
          )),
        ])),
        Text('${_currentPage + 1} / $_totalPages', style: GoogleFonts.barlow(
          fontSize: 14.sp, fontWeight: FontWeight.w600, color: kMutedText,
        )),
      ]),
    );
  }
  ...
  Widget _page1(InputNotifier inp, ImageNotifier img) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        _field(_catIdCtrl, 'Catalogue Identifier', 'e.g. RBA-001', Icons.tag_rounded, 
            onChanged: inp.setCatalogueIdentifier),
        SizedBox(height: 12.h),
        _field(_toolNameCtrl, 'Tool Name', 'e.g. Baileigh RB-800', Icons.label_outline_rounded,
  ...
  Widget _page2(InputNotifier inp) {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(height: 12.h),
        _field(_mfrCtrl,     'Manufacturer',      'e.g. Baileigh Industrial', Icons.factory_outlined),
        SizedBox(height: 12.h),
        _field(_countryCtrl, 'Country of Origin', 'e.g. United States',       Icons.flag_outlined),
        SizedBox(height: 12.h),
        _field(_eraCtrl,     'Era of Production', 'e.g. 1970',  Icons.history_rounded,
        ...
        SizedBox(height: 12.h),
        _field(_modelCtrl,  'Model Number',  'e.g. RB-800',      Icons.numbers_rounded),
        SizedBox(height: 12.h),
        _field(_serialCtrl, 'Serial Number', 'e.g. SN-19741023', Icons.qr_code_rounded),
        SizedBox(height: 24.h),
      ]),
    );
  }
  ...
  Widget _page3() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        SizedBox(height: 12.h),
        _field(_rebarCapCtrl,  'Rebar Size Capacity', 'e.g. #3 – #8, 6mm – 25mm', Icons.straighten_rounded),
        SizedBox(height: 12.h),
        _field(_bendAngleCtrl, 'Bend Angle Range',    'e.g. 0° – 180°',            Icons.rotate_left_rounded),
        SizedBox(height: 12.h),
        _field(_materialCtrl,  'Primary Material',    'e.g. Cast Iron, Steel',     Icons.hardware_rounded),
        SizedBox(height: 12.h),
        _field(_weightCtrl,    'Weight',              'e.g. 45 kg',                Icons.scale_rounded),
        SizedBox(height: 20.h),
        _sectionLabel('COLLECTOR NOTES'),
        SizedBox(height: 12.h),
        _field(_provenanceCtrl, 'Provenance / Acquisition', 'Where / how you acquired it', Icons.receipt_long_outlined),
        SizedBox(height: 12.h),
        _field(_notesCtrl,      'Notes',  'Any additional notes…',           Icons.notes_rounded,      maxLines: 3),
        SizedBox(height: 12.h),
        _field(_tagsCtrl,       'Tags',   'e.g. industrial, vintage, rare',  Icons.local_offer_outlined),
        SizedBox(height: 24.h),
      ]),
    );
  }
      ...
      child: Container(
        ...
        child: img.resultImage != null
            ? ClipRRect(
                ...
                child: Image.file(File(img.resultImage!), fit: BoxFit.cover,
  ...
  Widget _photoPlaceholder() {
    return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(
        ...
        decoration: BoxDecoration(color: kAccentMuted, borderRadius: BorderRadius.circular(kRadiusLarge)),
        child: Icon(Icons.add_a_photo_outlined, color: kAccent, size: 24.r),
      ),
      SizedBox(height: 12.h),
      Text('Tap to add a photo', style: GoogleFonts.barlow(
        fontSize: 14.sp, fontWeight: FontWeight.w600, color: kSecondaryText,
      )),
    ]);
  }
  ...
  Widget _sectionLabel(String label) {
    return Row(children: [
      ...
      Text(label, style: GoogleFonts.barlow(
        fontSize: 10.sp, fontWeight: FontWeight.w800, color: kAccent, letterSpacing: 2.0,
      )),
    ]);
  }
  ...
  Widget _chipSelector<T>({
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(
        ...
        child: Text(label, style: GoogleFonts.inter(fontSize: 12.sp, color: kSecondaryText)),
      ),
      SizedBox(
        height: 38.h,
          ...
          itemBuilder: (_, i) {
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                child: Text(getName(v), style: GoogleFonts.inter(
                  fontSize: 12.sp,
  ...
  Widget _conditionSelector(InputNotifier inp) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Padding(
        padding: EdgeInsets.only(left: 4.w, bottom: 8.h),
        child: Text('Condition', style: GoogleFonts.inter(fontSize: 12.sp, color: kSecondaryText)),
      ),
      SizedBox(
        height: 38.h,
        child: ListView.separated(
            ...
            return GestureDetector(
              ...
              child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(kRadiusPill),
                  border: Border.all(color: sel ? color : kOutline,
                      width: sel ? kStrokeWeightMedium : kStrokeWeightThin),
                ),
  ...
  Widget _navBar() {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(
        border: Border(top: BorderSide(color: kOutline, width: kStrokeWeightThin)),
      ),
      child: Row(children: [
        if (_currentPage > 0)
          GestureDetector(
            ...
            child: Container(
              ...
              child: Icon(Icons.arrow_back_rounded, color: kSecondaryText, size: 20.r),
            ),
          ),
        Expanded(
            ...
            child: AnimatedOpacity(
              ...
              child: Container(
                ...
                child: Center(child: Text(
                  isLast ? (widget.isEdit ? 'SAVE CHANGES' : 'ADD TO ARCHIVE') : 'CONTINUE',
                  style: GoogleFonts.barlow(fontSize: 14.sp, fontWeight: FontWeight.w800,
                      color: kBackground, letterSpacing: 1.0),
                )),
              ),
            ),
          ),
